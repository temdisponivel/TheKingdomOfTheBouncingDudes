...
namespace BounceDudes
{
    ...
    public class Character : MonoBehaviour
    {
        ...
        public void StartMoveAnimation(Vector3 destination, float speed)
        {
            this._movingAnimation = true;
            this._positionToMove = destination;
            this._animationSpeed = speed;
        }

        public void StopMoveAnimation()
        {
            this._movingAnimation = false;
        }

        protected void MoveTo()
        {

            // If arrived in destination, stop.
            if (this.transform.position == this._positionToMove)
            {
                this.StopMoveAnimation();
                return;
            }

            this.transform.position = Vector3.MoveTowards(this.transform.position, this._positionToMove, this._animationSpeed * Time.deltaTime);
